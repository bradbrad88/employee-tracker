...
// This function looks at the prompts we are about to provide to inquirer.
// If the prompts are a menu (name=MENU_ITEM and values hold a reference to functions) then add a 'back' menu item with a reference to the last menu as a function.
// Also add a 'back' property to the prompts, so the function that runs has access to the back() function and can end its process by landing at the previous menu (rather than go back to the main menu).
